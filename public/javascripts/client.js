...
var Game = {
    players: {},
    enemies: {},
    socket: io.connect('http://192.168.0.2:9696'),
	id: null,
	stars:[],
	suns: [],
	...
	level: 0,
	fighter: new Image(),
	fighters: [{img:"../images/fighter.png"},{img:"../images/fighter1.png"}],
	sys: {
/*
		n1: {
			img: '../images/n1.png',
	...
	init: function(user, ship) {
		...
		document.addEventListener('keydown', function (event) {
           ...
           if(event.keyCode == 32) {
               console.log();
               if(self.players[self.id].shots.length-1 > 0) {
                  self.player_missile.play();
               }
           }
    ...
    renderShip: function () {
    	...
    	if(this.id) {
    	    ...
    	    if(screen) {
    	    	if(thisPlayer) {
    	    	    window.scroll(thisPlayer.x-500, thisPlayer.y-500);
    	    	}
    	    }	   
		}	   
...
function go() {
	...
	if(document.getElementById('fighter').checked) {
	   ship = document.getElementById('fighter');
	} else {
	   ship = document.getElementById('fighter1');	
	}	
    
    if(user.value == "Enter player name") {
    	user.value = "Guest"
    }
    document.getElementById('setup').style.display = "none"; 
	Game.init(user.value, ship.value);   
